...
public class UserQueueExtension implements BeforeEachCallback, AfterTestExecutionCallback, ParameterResolver {
        ...
        for (Parameter parameter : parameters) {
            if (parameter.getType().isAssignableFrom(UserJson.class)) {
                ...
                while (candidateForTest == null) {
                    candidateForTest = usersQueueByType.poll(); // TODO нужен тут sleep ?
                }
                candidateForTest.setUserType(userType); //FIXME наверное правильнее выставить тип во время инициализации
    ...
    @Override //FIXME Почему запускается дважды подряд для каждого параметра ?
